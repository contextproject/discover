...
public class CommentRetrieverTest {
  ...
  /**
   * Checking the getcommentswithcontest function.
   * 
   * @throws Exception
   *           if something goes wrong.
   */
  ...
  public void getCommentsWithContentTest() throws Exception {
    databaseConnector.executeUpdate("INSERT INTO with_features_comments "
        + "VALUES (1, 1, 1, '2015-01-01 12:00:00', 1, 'test')");
    HashMap<Comment, String> comments = (HashMap<Comment, String>) commentRetriever
        .getCommentsWithString(1);
    ...
    databaseConnector.executeUpdate("DELETE FROM with_features_comments WHERE track_id = 1 "
        + "AND comment_id = 1 AND user_id = 1 AND created_at = '2015-01-01"
        + " 12:00:00' AND timestamp = 1 AND text = 'test'");
  }
}
